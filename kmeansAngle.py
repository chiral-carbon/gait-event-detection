...
if __name__ == "__main__":
    ...
    stance, swing, st, sw = StSw(hs_pos, to_pos, t) #stance and swing phase frame limits for all cycles from velocity based algorithm
    ...
    Xaf = np.array(f2.loc[:, 'ankle_angle_l'])
    ta = np.array(f2.loc[:, 'time'])
    frame2 = np.array(f2.loc[:, 'frame'])
    Vaf = [0]
    
    for i in range(len(Xaf)-1):
        Vaf.append((Xaf[i+1]-Xaf[i])/(ta[i+1]-ta[i])) #Velocity data - 3 point estimation

    dim = 2 #2 features for stance and swing phase clustering(K=2); 1. left ankle angle data, 2. angular velocity data
    K = input("No. of clusters: ")
    I = input("No. of iterations: ")
    l = len(Xaf)
    data = np.zeros(shape=(l,dim))
    for i in range(l):
        data[i] = np.array([Xaf[i], Vaf[i]])
	...
	for j, val in enumerate(range(hs_pos[i], hs_pos[i+1])):  #one gait cycle - from HS to frame before next consecutive HS
	    cyc[i][j] = np.array([Xaf[j], Vaf[j]]) #cyc[i] is (i+1)th gait cycle and contains data in this gait cycle. data:(angle, angular_vel)
    ...
    if c==True:
        ...
        myData.append(["Frame","Time","Filtered AnkleLeftX ", "VelocityX", "Left Ankle Angle", "Angular Velocity", "Phase from Velocity algo", "Phase from Kmeans", "Gait Event from k-means"])
        for i in range(len(data)):
            myData.append([frame[i], t[i], Xf[i], Vf[i], Xaf[i], Vaf[i], y_true[i], y_pred[i], event[i]])
        myFile = open(filename+'_stance and swing from KMeans and Velocity algo for angle data.csv','w')
        with myFile:
    ...
    if c==True:
	plot(cyc[0], mu, cp, j, K, I, dim, Xf, Vf, hs[0], to[0], J)
