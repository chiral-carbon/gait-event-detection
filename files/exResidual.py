...
def residual_analysis(y, freq=1, fclim=[], ax=None):
    """ Automatic search of filter cutoff frequency based on residual analysis.
    ...
    considerably bellow the Nyquist frequency and the noise is predominantly
    ...
    freqs = np.arange(0.1, 10.1, 0.1)
    res = []
    for fc in freqs:
        b, a = butter(2, (fc/C) / (freq / 2), btype = 'low')
        yf = filtfilt(b, a, y)
    ...
    try:
        ...
        if fc_opt:
            ...
            ax[0].plot(fc_opt, 0, 'ro', markersize=7, clip_on=False,
                       zorder=9, label='$Fc_{opt}$ = %.2f Hz' % fc_opt)
...
if __name__ == "__main__":
    ...
    y = np.array(data.loc[:, 'AnkleLeftX'])
    yf, fc_opt = residual_analysis(y, freq=45, show = 'True')
    print(fc_opt)
