...
def residual(y, fs, show):
    ...
    if len(fclim) and fclim[0] < fclim[1]:
        ...
        # optimal cutoff frequency is thes frequency where y[fc_opt] = A
...
if __name__ == "__main__":
    ...
    show = input("Plot the graph of residual vs frequency? \nType True for yes and False for no: ")
    filtered = residual(y, f, show)

  
    c = input("Create new csv for interpolated values and another for filtered and unfiltered values? \nType True for yes and False for no: ")
    if c==True:
        #Creating csv with interpolated values using Cubic Spline interpolation 
        myData = []
        myData.append(["Frame","Time","AnkleLeftX - SpineBaseX"])
        for i in range(len(y)):
            myData.append([frame[i], time[i], y[i]])
        myFile = open(filename+'_new.csv','w')
        with myFile:
            writer = csv.writer(myFile) 
            writer.writerows(myData)

        myData = []
        myData.append(["Frame","Time","AnkleLeftX - SpineBaseX", "Filtered"])
        for i in range(len(y)):
