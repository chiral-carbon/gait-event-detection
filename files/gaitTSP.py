...
def StSw(hs_li, to_li, time):
	...
	for i in range(len(hs)):
	    swing.append([to[i], hs[i]])
            swt.append(time[hs[i]] - time[to[i]])
	for i in range(len(to)-1):
	    stance.append([hs[i], to[i+1]])
            stt.append(time[to[i+1]] - time[hs[i]])
    else:
	for i in range(len(to)):
	    swing.append([to[i], hs[i]])
            stt.append(time[to[i]] - time[hs[i]])
    ...
    return stance, swing, round(np.mean(stt),3), round(np.mean(swt),3)
